...
void compressor(int hr , int minute)
{
  ...
  if (distance > 8 && digitalRead(comp) != 1 && (hr >= 23 || hr <= 5)) // dalay to turn on the compressor is ten sec for now and will be changed after according to the condition
  {
  ...
  else if (digitalRead(comp) != 0 && ((distance < 5 ) || (hr < 23 && hr > 5) ))
  {
    Serial.println(hr);
    Serial.println(minute);
    turnOffComp();
  ...
  else if (digitalRead(fan) == 1 && digitalRead(comp) == 0 && interval > 120000 && (distance < 8 || (hr < 23 && hr > 5)))
  {
