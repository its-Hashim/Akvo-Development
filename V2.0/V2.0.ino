...
void compressor(int humid, int temp)
{
  ...
  if (distance > 7 && distance < 40 && digitalRead(comp) != 0 && interval > 60000 && humid > 30 && temp >15 && temp <50) // dalay to turn on the compressor is ten sec for now and will be changed after according to the condition
  {
  ...
  else if (digitalRead(comp) != 1 && (distance < 4 || distance > 40 || humid <=30  || temp <=15 || temp >= 50) )
  {
