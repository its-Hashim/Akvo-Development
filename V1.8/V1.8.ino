...
void setup()
{
  ...
  else
  {
    ...
    f.println("\n\n\n\n BOOTED\n\n\n\n");
    f.close();
    Serial.println("somefilsomefile.txt created hooooowa");
  }
  delay(1000);
  //Setting display
...
void loop()
{
  ...
  if (interval > 3000)
  {
    ...
    compressor();
    lastChecked = millis();
    store(t,d,int(dht1.readTemperature()),int(dht1.readHumidity() - 3));
...
void store(String t, String d, int ambTemp, int ambHumid)
{
  if (SD.exists("somefile.txt"))
  {
    Serial.println("storing...");
    f = SD.open("somefile.txt", FILE_WRITE);
    f.println("The time is now " + t);
    f.println("Todays date is " + d);
    f.println("Ambient Temperature is " + String(ambTemp));
    f.println("Ambient Temperature is " + String(ambHumid));
